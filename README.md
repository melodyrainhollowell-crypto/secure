...
Site de ebooks para hospedagem no Render. Funciona como **front/máscara** do checkout (**Whop** + PayPal + **Paddle Billing**) — quando o provedor verifica a origem, vê este domínio de ebooks.
...
O fluxo continua: o cliente sai do catálogo de vídeos e abre o checkout **neste** domínio (ebooks), com `method=whop` (padrão), `method=paypal` ou `method=paddle`.
...
- `GET /api/paypal-checkout?...&method=whop|paypal|paddle` — Whop (API + redirect whop.com), PayPal in-page ou Paddle (transação via API + overlay Paddle.js)
